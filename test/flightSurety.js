...
//     let funds = await config.flightSuretyApp.fundAirline.call({from: config.firstAirline, value: amountInWei});
//     console.log(funds);
...
//   });
  ...
  it('(airline) unregistered airline cannot register new airline', async() => {
    let result = true;
    try {
        result = await config.flightSuretyApp.registerAirline.call(config.fourthAirline, {from: config.thirdAirline});
    } catch {
        result = false;
    }
